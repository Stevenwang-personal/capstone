...
%load data file path
%Data=load('infExportData_I_UWB_LPS_Tag_Data_Frame0_20180531_000541.txt');
...
% figure;
% subplot(3,3,1),plot((1:N),w(:,1),'b');grid on,xlabel('sample num'),ylabel('rad/s'),title('角速度w.x');
...
% subplot(3,3,4),plot((1:N),a(:,1),'b');grid on,xlabel('sample num'),ylabel('m^2/s'),title('加速度a.x');
...
% subplot(3,3,7),plot((1:N),angle(:,1));grid on,xlabel('sample num'),ylabel('deg'),title('角度angle.x');
